...
use crate::{helper::buffer_to_array, Command, Db, Listener};
...
impl Handler {
    ...
    pub async fn process_query(
        ...
        match command {
            Command::Get => {
                let result = self.db.read(&attrs);
                match result {
                    Ok(result) => {
                        connection.stream.write_all(&result).await?;
                    }
                    Err(err) => {
                        connection.stream.write_all(err.as_bytes()).await?;
                    }
                }
                return Ok(());
            }
            Command::Set => {
                let resp = self.db.write(&attrs);
                match resp {
                    Ok(result) => {
